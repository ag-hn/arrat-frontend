...
import { AvailableChartColors, type AvailableChartColorsKeys, getColorClassName } from "./shared/chart-utils"
...
const BarLabels = ({ values }: { values: number[] }) => {
  ...
  return (
    <div
      className={cn(
        // base
        "relative mb-2 flex h-5 w-full text-xs font-medium",
        // text color
      ...
      {values.map((widthPercentage, index) => {
        ...
        return (
          ...
          >
            {showLabel ? (
              <span
                className={cn("block translate-x-1/2 tabular-nums")}
              >
...
const ChartCategoryBar = React.forwardRef<HTMLDivElement, CategoryBarProps>(
  (
    {
      values = [],
...
ChartCategoryBar.displayName = "CategoryBar"

export { ChartCategoryBar, type CategoryBarProps }
